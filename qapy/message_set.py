...
class AntMessageType:
    ...
    def test(self, message): # message is list-formatted Ant message
        ...
        if len(message_values)!=len(test_values): raise
    ...
    def __repr__(self):
        return '\'%s\': { %s }'%(self.name,', '.join(["'%s':%s"%(k,self[k]) for k in self.keys() if not k[0]=="_"]))
...
class MessageSet:
    ...
    def check_rssi_message(self,message):
        ...
        if len(m_c)==18 and m_c[0] in [0xc1, 0xc2, 0xc3]:
            channel_number=m_c[1]
            device_number=m_c[2]+(m_c[3]<<8)
            device_type_id=m_c[4]
            transmission_type=m_c[5]
            set_last_power(message[6:10])
            replacement_message=[m_c[0]-(0xc1-0x4e),m_c[1]]+m_c[10:]
            return self._new_message(''.join(map(chr,replacement_message)))

        elif m_c[0] in [0xc1, 0xc2, 0xc3]:
            print "bingo",m_c
            raise

        elif len(m_c)==8 and m_c[0]==0x40:
            set_last_power(message[4:8])
            replacement_message=m_c[0:3]+[5] # 0x10 --> 0x05
            return self._new_message(''.join(map(chr,replacement_message)))
        else:
            return self._new_message(message)            
